              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-slate-100">
                  <div>
                    ...
                    <p className={`text-lg font-bold ${(member.total ?? 0) === 0 ? 'text-slate-400' : (member.onTimeRate ?? 0) >= 0.8 ? 'text-emerald-600' : (member.onTimeRate ?? 0) >= 0.5 ? 'text-amber-600' : 'text-rose-600'}`}>
                      {(member.total ?? 0) === 0 ? '—' : `${Math.round((member.onTimeRate ?? 0) * 100)}%`}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 font-medium">Overdue / Delayed</p>
                    <p className={`text-lg font-bold ${(member.overdue ?? 0) > 0 ? 'text-rose-600' : (member.total ?? 0) === 0 ? 'text-slate-400' : 'text-emerald-600'}`}>
                      {(member.total ?? 0) === 0 ? '0' : (member.overdue ?? 0)}
                    </p>
                  </div>
                </div>
              </Link>
