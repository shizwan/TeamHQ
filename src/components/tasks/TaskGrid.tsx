...
type SortField = 'createdAt' | 'targetDueDate' | 'completedAt' | 'delayHours' | 'title' | 'deliverableId' | 'status' | 'assignee' | 'project' | 'daysActive';
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'completedAt': {
          const dateA = a.completedDate || a.completedAt ? new Date(a.completedDate || a.completedAt!).getTime() : 0;
          const dateB = b.completedDate || b.completedAt ? new Date(b.completedDate || b.completedAt!).getTime() : 0;
          comparison = dateA - dateB;
          break;
        }
        case 'delayHours':
                ...
                {currentTasks.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3 px-3 whitespace-nowrap">
                        {t.status === 'Completed' ? (
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-bold bg-emerald-50 text-emerald-800 border border-emerald-200/80" title={`Completed at ${t.completedTime || '10:00 PM'}`}>
